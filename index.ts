...
export const closeATA = async (baseMint: PublicKey, wallet: Keypair) => {
  try {
    const tokenAta = await getAssociatedTokenAddress(baseMint, wallet.publicKey)
    // Create the close account instruction
    const closeInstruction = createCloseAccountInstruction(
      tokenAta,       // token account to close
    ...
    console.log(`Token account closed successfully. Transaction signature: ${signature}`);
    return signature;
  } catch (error) {
